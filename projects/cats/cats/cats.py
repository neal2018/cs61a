...
def accuracy(typed, reference):
    ...
    cnt = 0
    for i, v in enumerate(typed_words):
        if i < n and reference_words[i] == v:
            cnt += 1
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(start, goal, limit, cur):
        if cur > limit:
            return limit + 1
        elif not start:
            return len(goal) + cur
        elif not goal:
            return len(start) + cur
        elif start[0] == goal[0]:
            return helper(start[1:], goal[1:], limit, cur)
        else:
            return helper(start[1:], goal[1:], limit, cur + 1)
...
def report_progress(typed, prompt, id, send):
    ...
    # BEGIN PROBLEM 8
    res = {'id': id, 'progress': 0}
    i = 0
    while i < len(typed) and typed[i] == prompt[i]:
        i += 1
    res['progress'] = i/len(prompt)
    send(res)
    return res['progress']
    # END PROBLEM 8
...
def time_per_word(times_per_player, words):
    ...
    times = [diff(t) for t in times_per_player]
    ...
    for w in words:
        ...
        for p in players:
            ...
            if t < min_time:
                min_time = t
                target_player = p
        res[target_player].append(word_at(game, w))
    return res
    # END PROBLEM 10
...
def key_distance_diff(start, goal, limit):
    ...
    if limit < 0:
        ...
        substitute_diff = key_distance_diff(
            start[:-1], goal[:-1], limit -1) + key_distance[start[-1], goal[-1]]
        return min(add_diff, remove_diff, substitute_diff)
    # END PROBLEM EC1
...
helper = memo(autocorrect)
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    """A memoized version of the autocorrect function implemented above."""

    # BEGIN PROBLEM EC2
    return helper(user_word, tuple(valid_words), diff_function, limit)
    # END PROBLEM EC2 
